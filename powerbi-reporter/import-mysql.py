...
def fetch_rc_health(api, asset_id):
    ...
    if 'assetEndpointInfo' in data:
        ...
        insert = """
            INSERT IGNORE INTO asset_health (id, color, actual_isolation_status, desired_isolation_status, last_module_status_time, 
...
#            tenant_id = asset['tenantId']
            ...
            insert = """
                INSERT IGNORE INTO asset (id, tenant_id, architecture, created_at, updated_at, deleted_at,
...
def fetch_investigations(api, tenant_id):
    ...
    if results is not None:
        for inv in results:
            ...
            id = inv['id']
            description = inv['description'].replace("'", r"\'")
...
#            else:
            ...
            acknowledge_time = inv['transition_state']['acknowledge_time']
            initial_acknowledge_time = inv['transition_state']['initial_acknowledge_time']
            acknowledged_at_least_once = inv['transition_state']['acknowledged_at_least_once']

            handoff_time = inv['transition_state']['handoff_time']
            initial_handoff_time = inv['transition_state']['initial_handoff_time']
            ...
            resolution_time = inv['transition_state']['resolution_time']
            initial_resolution_time = inv['transition_state']['initial_resolution_time']
            resolved_at_least_once = inv['transition_state']['resolved_at_least_once']
            
            ## Insert investigation into table
            
            insert = """
                    INSERT IGNORE INTO investigation (id, description, latest_activity, priority, status, type,
...
#            tenant_id = alert['tenant_id'][0]
            ...
            ## Insert into database
            insert = """
                    INSERT IGNORE INTO alert(id, alert_type, confidence, severity, creator, tenant_id, message, 
            ...
            if asset_id is not None:
                insert = """
                        INSERT IGNORE INTO alert_asset(alert_id, asset_id, tenant_id)
                        VALUES ('%s', '%s', %s);
...
def parse_tags(api, tenant_id):
    ...
    for asset in c.fetchall():
        ...
        for tag in tags.split(","):
            if not tag == "":
                sql = """
                    INSERT IGNORE INTO asset_tag(asset_id, tag, tenant_id) VALUES ('%s', '%s', %i);
